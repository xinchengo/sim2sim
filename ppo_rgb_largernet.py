...
class NatureCNN(nn.Module):
    def __init__(self, sample_obs):
        ...
        self.out_features = 0
        feature_size = 512  # Increased from 256 to 512
        in_channels=sample_obs["rgb"].shape[-1]
        image_size=(sample_obs["rgb"].shape[1], sample_obs["rgb"].shape[2])


        # here we use a larger NatureCNN architecture to process images
        cnn = nn.Sequential(
            nn.Conv2d(
            ...
            nn.ReLU(),
            nn.Conv2d(
                in_channels=64, out_channels=128, kernel_size=4, stride=2, padding=0  # Increased from 64 to 128
            ),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=128, out_channels=128, kernel_size=3, stride=1, padding=0  # Increased from 64 to 128
            ),
            nn.ReLU(),
            nn.Conv2d(
                in_channels=128, out_channels=256, kernel_size=3, stride=1, padding=0  # Added new layer
            ),
            nn.ReLU(),
            nn.Flatten(),
        )
        ...
        with torch.no_grad():
            ...
            fc = nn.Sequential(
                nn.Linear(n_flatten, 1024),  # Increased intermediate size
                nn.ReLU(), 
                nn.Linear(1024, feature_size),  # Added another layer
                nn.ReLU()
            )
        ...
        if "state" in sample_obs:
            # for state data we use a larger multi-layer network
            state_size = sample_obs["state"].shape[-1]
            extractors["state"] = nn.Sequential(
                nn.Linear(state_size, 512),  # Increased from 256 to 512
                nn.ReLU(),
                nn.Linear(512, 512),  # Added another layer
                nn.ReLU()
            )
            self.out_features += 512  # Updated to match new state feature size
...
class Agent(nn.Module):
    def __init__(self, envs, sample_obs):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(latent_size, 1024)),  # Increased from 512 to 1024
            nn.ReLU(inplace=True),
            layer_init(nn.Linear(1024, 512)),  # Added intermediate layer
            nn.ReLU(inplace=True),
